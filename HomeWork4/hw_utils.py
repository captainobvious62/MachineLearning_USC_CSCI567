...
def testmodels(X_tr, y_tr, X_te, y_te, archs, actfn='relu', last_act='softmax', reg_coeffs=[0.0],
    ...
    for arch in archs:
        print " Starting for arch : ", arch
        start_time = time.time()
        for reg_coeff in reg_coeffs:
            print " For Reg : ", reg_coeff
            for sgd_decay in sgd_decays:
                print " For decay : ", sgd_decay
                for sgd_mom in sgd_moms:
                    print " For momentum : ", sgd_mom
                    # Generate Model
                    ...
                    print('Score for architecture = {0}, lambda = {1}, decay = {2}, momentum = {3}, actfn = {4}, accuracy : {5}'.format(arch, reg_coeff, sgd_decay, sgd_mom, actfn, score[1]))
                    result = Data.Data(arch, score[1], time.time() - start_time)
    ...
    #print " Reached here"
    results.append(Data.Data(best_config[0], 0, 0))
    return results